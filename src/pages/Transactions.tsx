...
import { Plus, Filter, Download, ScanLine } from 'lucide-react'
import { format } from 'date-fns'
import ReceiptScanner from '@/components/ReceiptScanner'
import type { ReceiptParseResult } from '@/services/ai'
...
export default function Transactions() {
  ...
  const handleReceiptParsed = (result: ReceiptParseResult, receiptId: string | null) => {
    setShowScanner(false)
    setPendingReceiptId(receiptId)

    // Auto-fill the form with parsed data
    setValue('amount', result.total.toString())
    setValue('category', result.suggested_category)
    setValue('date', result.date)
    setValue('note', `${result.vendor}${result.line_items.length > 0 ? ' — ' + result.line_items.map(i => i.description).join(', ') : ''}`)
    setValue('type', 'expense')

    // Open the transaction form so the user can review
    setShowModal(true)
    toast.info('Receipt data loaded! Review and save the transaction.')
  }
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="px-4 py-2 bg-white/5 border border-white/10 text-neutral-300 hover:text-white hover:bg-white/10 rounded-lg transition flex items-center space-x-2"
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-black/90 border border-white/10 rounded-lg shadow-2xl p-6 w-full max-w-md">
            <h3 className="font-bold text-2xl text-white mb-6">
              {pendingReceiptId ? 'Review Scanned Transaction' : 'Add Transaction'}
            </h3>
            {pendingReceiptId && (
              <div className="mb-4 p-3 bg-purple-500/10 border border-purple-500/30 rounded-lg">
                <p className="text-purple-300 text-sm flex items-center space-x-2">
                  <ScanLine className="w-4 h-4" />
                  <span>Auto-filled from receipt scan. Review and edit if needed.</span>
                </p>
              </div>
            )}
